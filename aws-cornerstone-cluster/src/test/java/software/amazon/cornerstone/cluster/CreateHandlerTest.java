...
import static software.amazon.cornerstone.cluster.CreateHandler.NODE_TYPE_REQUIRED_FOR_CLUSTER;
import static software.amazon.cornerstone.cluster.CreateHandler.SUBNET_GROUP_REQUIRED;
...
import software.amazon.awssdk.services.cornerstone.model.CreateClusterResponse;
...
public class CreateHandlerTest extends AbstractTestBase {
    ...
    private CreateClusterResponse getCreateClusterReponse() {
        return CreateClusterResponse.builder().name(CLUSTER_NAME).description(CLUSTER_DESCRIPTION).status(CREATING_STATUS).nodeType(NODE_TYPE)
                                    .numShards(NUM_SHARDS).subnetGroupName(SUBNET_GROUP_NAME).securityGroups(getSecurityGroupMemberships(SECURITY_GROUP_IDS))
                                    .snsTopicArn(SNS_TOPIC_ARN).tlsEnabled(TLS_ENABLED).arn(CLUSTER_ARN).engineVersion(ENGINE_VERSION)
                                    .clusterEndpoint(software.amazon.awssdk.services.cornerstone.model.Endpoint.builder().port(PORT).build())
                                    .availabilityMode(AVAILABILITY_MODE).build();
    }
}
