...
public class DeleteHandler extends BaseHandlerStd {
    ...
    protected ProgressEvent<ResourceModel, CallbackContext> handleRequest(
        ...
        return ProgressEvent.progress(request.getDesiredResourceState(), callbackContext)
            .then(progress -> checkUserGroupIsAssociated(proxy, proxyClient, progress, request))
            .then(progress -> deleteUserGroup(proxy, proxyClient, progress, request, logger));
    }

    protected ProgressEvent<ResourceModel, CallbackContext> checkUserGroupIsAssociated(
        final AmazonWebServicesClientProxy proxy,
        final ProxyClient<MemoryDbClient> proxyClient,
    ...
    private ProgressEvent<ResourceModel, CallbackContext> deleteUserGroup(final AmazonWebServicesClientProxy proxy,
        final ProxyClient<MemoryDbClient> proxyClient,
        final ProgressEvent<ResourceModel, CallbackContext> progress,
        final ResourceHandlerRequest<ResourceModel> request,
        final Logger logger) {
